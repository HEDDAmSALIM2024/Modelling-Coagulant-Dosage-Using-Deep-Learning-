...
# Reshaping pour BiGRU (n_samples, n_timesteps, n_features)
X_train_reshaped = X_train.reshape((X_train.shape[0], X_train.shape[1],1))  # (samples, timesteps, features)
X_test_reshaped  = X_test.reshape((X_test.shape[0],   X_test.shape [1],1))

# Définir le modèle BiGRU
model =  tf.keras.Sequential()
opt = Adam(learning_rate=0.005)
model.add(Bidirectional(GRU(150, dropout=0.1, recurrent_dropout=0.1,return_sequences=False), input_shape=(10, 1)))  # 6 étapes de temps, 1 caractéristique par étape
...
Y_pred_original1 = scaler_Y.inverse_transform(Y_pred_train)
Y_train_original = scaler_Y.inverse_transform(Y_train)
Y_pred_original2 = scaler_Y.inverse_transform(Y_pred_val)
...
print_performance_metrics(Y_train_original, Y_pred_original1, "Training")
print_performance_metrics(Y_val_original,   Y_pred_original2,   "Validation")
