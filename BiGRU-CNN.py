...
from tensorflow.keras.layers import Dense, LSTM,Bidirectional,GRU, Conv1D, Flatten, Dropout, MaxPooling1D
...
X_train = data.iloc[:, [0,1,2,5,6,7,8,9]].values  # Caractéristiques (2ème et 5ème colonnes)
Y_train = data.iloc[:, 10].values      # Cible 
X_test = data_val.iloc[:, [0,1,2,5,6,7,8,9]].values
Y_test = data_val.iloc[:, 10].values
"""
...
# Reshaping pour Conv1D et BiGRU-CNN (n_samples, n_timesteps, n_features)
...
# Partie CNN
model.add(Conv1D(filters=256, kernel_size=1, activation='relu', input_shape=(X_train_reshaped.shape[1], 1)))
model.add(MaxPooling1D(pool_size=2))
model.add(Dropout(0.2))

# Partie BIGRU
model.add(Bidirectional(GRU(150, dropout=0.2, recurrent_dropout=0.1,return_sequences=False), input_shape=(2, 1)))
...
# Entraîner le modèle
history = model.fit(X_train_reshaped, Y_train, epochs=100, batch_size=10, validation_data=(X_test_reshaped, Y_test))
