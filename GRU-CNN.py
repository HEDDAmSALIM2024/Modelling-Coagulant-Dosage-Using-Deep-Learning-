...
# Préparation des données (les 10 premières colonnes comme caractéristiques et la dernière comme cible)
...
# Reshaping pour Conv1D et GRU (n_samples, n_timesteps, n_features)
X_train_reshaped = X_train.reshape((X_train.shape[0], X_train.shape[1], 1))
X_test_reshaped  = X_test.reshape ((X_test.shape[0],  X_test.shape [1], 1))

# Définir le modèle GRU-CNN
model = Sequential()
...
# Partie GRU
model.add(GRU(300, dropout=0.1, recurrent_dropout=0.1,return_sequences=False, input_shape=(10, 1))) 
# Couche Dense
