...
from tensorflow.keras.layers import Dense,  Bidirectional, LSTM, Conv1D, Flatten, Dropout, MaxPooling1D
...
# Définir le modèle CNN-BiLSTM
model = Sequential()

# Partie CNN
model.add(Conv1D(filters=264, kernel_size=1, activation='relu', input_shape=(X_train_reshaped.shape[1], 1)))
model.add(MaxPooling1D(pool_size=2))
model.add(Dropout(0.2))

# Partie BiLSTM
model.add(Bidirectional(LSTM(100, dropout=0.10, recurrent_dropout=0.10,return_sequences=False), input_shape=(10, 1))) 
