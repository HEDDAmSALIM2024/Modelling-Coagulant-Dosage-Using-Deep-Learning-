...
# Reshaping pour LSTM (n_samples, n_timesteps, n_features)
X_train_reshaped = X_train.reshape((X_train.shape[0], X_train.shape[1],1))  # (samples, timesteps, features)
X_test_reshaped  = X_test.reshape((X_test.shape[0],   X_test.shape [1],1))

# Définir le modèle LSTM
model = Sequential()
model.add(LSTM(100, dropout=0.1, recurrent_dropout=0.1, return_sequences=False, input_shape=(X_train_reshaped.shape[1], 1)))
model.add(Dense(1))
